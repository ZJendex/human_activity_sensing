...
Build the system in two primary lanes, with one optional external benchmark:

1. **ZED baseline lane:** use live cameras, an SDK-native H.265 side-by-side stream, or SVO2 with the ZED SDK Body Tracking and Fusion modules to validate the rig quickly.
2. **Research lane:** process the three custom side-by-side elementary H.265 streams and implement the modular M0a-M6 pipeline below. This lane exposes intermediate measurements, uncertainty, and failure cases, and allows individual methods to be replaced.
3. **Optional Theia3D pilot:** export six synchronized rectified eye videos and matching custom intrinsics after M0b. This is an experimental comparison, because ZED X is not a camera system verified by Theia Markerless and the three stereo pairs provide only three substantially different viewpoints.
...
For future recordings, there are two supported choices:

1. Keep the custom H.265 path, but write a lossless per-frame metadata sidecar and a calibration snapshot.
2. Also record SVO2 with H.265 when storage permits. SVO2 preserves camera timestamps and sensor metadata and can be replayed through the ZED SDK.

Stereolabs documents a third case: H.265 generated by the ZED SDK streaming API is a side-by-side stereo stream that a receiver can open as a normal ZED input, retaining depth and other SDK modules. The current recording is also side-by-side, but it is a custom elementary stream and metadata format; do not assume SDK input compatibility until a short replay test succeeds. See [ZED local video streaming](https://docs.stereolabs.com/docs/development/zed-sdk/modules/camera/local-network-streaming) and [SVO2 recording](https://docs.stereolabs.com/docs/development/zed-sdk/modules/camera/recording).
...
| Array extrinsics `T_world_left` | Common world coordinates | ZED360 or target calibration |
...
### Two array-calibration paths

#### Path A: ZED360 quick baseline

[ZED360](https://www.stereolabs.com/docs/fusion/zed360) uses body-tracking keypoints to align cameras in a shared world frame and exports the Fusion JSON. It is the quickest way to make the ZED SDK Fusion baseline operational.

Important behavior documented by Stereolabs:

- Pitch and roll come from the camera IMU; yaw is calibrated.
- Floor level is estimated if ankles are visible.
- The first loaded camera defines the world origin.
- The calibration remains valid only while cameras remain fixed.
- The output JSON stores per-serial input configuration and camera world pose.

Limitations:

- It depends on body detections, so its calibration quality is coupled to a perception model.
- It is less transparent than a measured-target bundle adjustment.
- A calibration that visually looks plausible can still contain centimeter-scale bias.

Use it to establish a baseline, then verify it with an independent target or wand.

#### Path B: target-based research calibration
...
10. **Export both forms.** Store a canonical calibration file and generate a ZED Fusion JSON through a tested coordinate conversion.
...
The ZED default image frame is right-handed with `x` right, `y` down, and `z` forward. The API supports other coordinate systems and units. The Fusion JSON is documented in meters and the ZED image coordinate convention; its reader performs conversion. See [ZED coordinate frames](https://docs.stereolabs.com/docs/development/zed-sdk/modules/positional-tracking/coordinate-frames) and [Fusion configuration files](https://docs.stereolabs.com/docs/development/zed-sdk/modules/fusion).
...
For fixed cameras, use the known initial transform and static-camera setting rather than allowing independent visual-inertial trackers to drift. See [ZED positional-tracking settings](https://docs.stereolabs.com/docs/development/zed-sdk/modules/positional-tracking/settings).
...
Run the initial M1 baseline on each ZED X's decoded and rectified left image. Retain the decoded right image for optional stereo depth. Running a second pose model on the right image is optional; its short baseline adds less geometric information than another camera location and doubles pose-inference cost.
...
The ZED Body Tracking module already outputs 2D and 3D keypoints, confidence, tracking identity, and optional body fitting. It remains a live baseline. It can also be replayed from SVO2 or received through the ZED SDK's side-by-side streaming input. Do not assume that the custom side-by-side elementary H.265 plus project-specific metadata is SDK-replayable merely because its pixel layout is side-by-side. See [ZED Body Tracking](https://docs.stereolabs.com/docs/development/zed-sdk/modules/body-tracking).
...
Stereo depth is now an optional subproject with two branches:

1. **SDK-native branch:** if the source is a ZED SDK H.265 side-by-side stream or SVO2, retrieve ZED depth and confidence normally.
2. **Custom-file branch:** replay each indexed side-by-side sample, split both eyes, rectify them, and estimate disparity externally. Use StereoSGBM as a transparent baseline, [RAFT-Stereo](https://arxiv.org/abs/2109.07547) or [IGEV-Stereo](https://arxiv.org/abs/2303.06615) as established learned comparisons, and [FoundationStereo](https://arxiv.org/abs/2501.09898) or [Fast-FoundationStereo](https://arxiv.org/abs/2512.11130) as current zero-shot research candidates.
...
7. Run ZED360 and the official Fusion sample live as a comparison if the custom side-by-side elementary streams cannot be used as SDK input.
8. Create the canonical calibration package and automated M0 tests.
...
5. Compare against the live/native ZED Fusion baseline on a newly captured common test sequence where possible.
...
|---|---|---|---|
| H.265 ingest | Hardware decode + external `sample_index`/`pts_ns` manifest + fixed split | Side-by-side SDK stream/SVO2 | Preserve the external index and shared stereo-sample identity |
...
| ZED baseline | Live/native ZED Body Tracking + Fusion | Target-calibrated custom pipeline | Generic eye files may not be SDK-replayable |
...
- [Stereolabs: Body Tracking](https://docs.stereolabs.com/docs/development/zed-sdk/modules/body-tracking)
...
- [Stereolabs: Positional Tracking Settings](https://docs.stereolabs.com/docs/development/zed-sdk/modules/positional-tracking/settings)
- [Stereolabs: Local H.264/H.265 Video Streaming](https://docs.stereolabs.com/docs/development/zed-sdk/modules/camera/local-network-streaming)
- [Stereolabs: Video Recording/SVO2](https://docs.stereolabs.com/docs/development/zed-sdk/modules/camera/recording)
